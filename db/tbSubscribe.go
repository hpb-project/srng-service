...
func (t *TbSubscribe) TableName() string {
	return "tb_subscribe"
}
...
// GetTbSubscribeById retrieves TbSubscribe by Id. Returns error if
// Id doesn't exist
func GetTbSubscribeById(id int64) (v *TbSubscribe, err error) {
	o := orm.NewOrm()
	v = &TbSubscribe{Id: id}
...
// GetAllTbSubscribe retrieves all TbSubscribe matches certain condition. Returns empty list if
// no records exist
func GetAllTbSubscribe(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(TbSubscribe))
	// query k=v
	for k, v := range query {
...
// UpdateTbSubscribe updates TbSubscribe by Id and returns error if
// the record to be updated doesn't exist
func UpdateTbSubscribeById(m *TbSubscribe) (err error) {
	o := orm.NewOrm()
	v := TbSubscribe{Id: m.Id}
...
// DeleteTbSubscribe deletes TbSubscribe by Id and returns error if
// the record to be deleted doesn't exist
func DeleteTbSubscribe(id int64) (err error) {
	o := orm.NewOrm()
	v := TbSubscribe{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&TbSubscribe{Id: id}); err == nil {
